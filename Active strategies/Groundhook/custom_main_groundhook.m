...
%     
    subplot(3,1,1)
    text = "";
    plot(cG_arr,Data.AVG(idxcar).Potact,'displayname',num2str(vcar_arra(idxcar)));
    hold on
    lg = legend('location','eastoutside');
    lg.Title.String = 'Velocity [km/h]';
    title("Road class C",'FontSize',20);
    xlabel("cGroundhook [Ns/m]")
    ylabel("Avg Power [W]")
    
    subplot(3,1,2)
    text = "";
    plot(cG_arr,Data.RMS(idxcar).etarh,'displayname',num2str(vcar_arra(idxcar)));
    hold on
    lg = legend('location','eastoutside');
    lg.Title.String = 'Velocity [km/h]';
    xlabel("cGroundhook [Ns/m]")
    ...
    subplot(3,1,3)
    text = "";
    plot(cG_arr,Data.RMS(idxcar).acel,'displayname',num2str(vcar_arra(idxcar)));
    hold on
    lg = legend('location','eastoutside');
    lg.Title.String = 'Velocity [km/h]';
    xlabel("cGroundhook [Ns/m]")
